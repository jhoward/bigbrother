...
%  \begin{algorithmic}
%    
%    \State $extracted_lengths \gets []$
%    \State $final_data \gets \[\]$
%    
%    \State \Comment{\#assume we have access to residual_data as a one dimensional array}
%    \For {$t \gets 0, 1$}
%    \State	window_sum = sum(absolute_value(residual_data\[$t$:$t + window_width$\]))
%    \State 	extrated_lengths.append(($window_sum$, $t$))
%    \EndFor
%    
%    \State \Comment{\#sort the extracted lengths by the window_sum dimension descending}
%    \State extracted_lengths = sort(extracted_lengths, dim1, order = "descending") 
%
%    \State num_to_extract = ceil(k * extracted_lengths.length)
%    
%    \State \Comment{\#Center all the extracted residuals by local peaks}
%    \For {$t = 0$; $t < num_to_extract$; $t++$}
%    \State 	data_index = extracted_lengths\[t\]\[1\]
%    \State	max_peak = find_max_peak(abs(residual_data[data_index:data_index + window_width]))
%
%    \State 	\Comment{\#Recenter the data around the local peak}
%    \State	final_data.append(residual_data[max_peak - window_width / 2: max_peak + window_width / 2])
%    \EndFor
%  
%  \end{algorithmic}
%
...
TODO EXTRACT PEAK FINDING ALGORITHM
