...
\ell(\phi, \mu, \Sigma) = \sum_{i = 1}^{M}\sum_{k = 1}^{K}w_{k}^{(i)} (\log \{ \frac{p(z^{(i)} = k)}{w_{k}}\} +  \log \{ \frac{\sum_{n = 1}^{N} \log \mathcal{N}_{n}(r^{(i)})}{w_{k}}\})
\end{equation}
...
\begin{equation}
w^{(i)}_{k} = p(z^{(i)} = j|r^{(i)}) = \frac{p(r^{(i)}|z^{(i)}=j)p(z^{(i)}=j)}
