...
\ref{fig:rmseplotdenver} and \ref{fig:rmseplotmerl} show the results of the root mean squared error (RMSE) of forecasts across a forecast horizon up to 15 time steps into the future for each model.  These plots show that BCF-TS generally has the lowest error.  However, the average model shows itself to be a strong indicator of future activity for forecasts beyond 6 to 8 time steps into the future.

TODO FINISH THIS ANALYSIS HERE
...
\begin{table}[t!]
\centering
\caption{RMSE forecast values per model for a horizon equal to two.}
\begin{tabular}{|c|c|c|c|c|c|c|} \hline
Dataset & ARIMA & TDNN & AVG & SVM & BCF & BCF-TS\\ \hline
Brown & 15.80 & 12.37 & 10.59 & 10.45 &  10.68 & \textbf{9.79}\\ \hline
MERL & 12.13 & 18.60 & 17.44 & 11.48 & 10.49 & \textbf{9.56}\\ \hline
Denver & 11.80 & 22.43 & 12.95 & 9.19 &  10.46 & \textbf{8.69}\\ \hline
\end{tabular}
\label{fig:rmsetab}
\end{table}
...
Plots for the results of RMSE values on the brown building dataset along with MASE values for both brown and MERL are included in the appendix (\ref{fig:rmseplotbrown}, \ref{fig:maseplotbrown}, and \ref{fig:maseplotmerl}).
