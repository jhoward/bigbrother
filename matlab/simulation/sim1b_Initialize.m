...
%All travel instances to classroom
actInstances = 4;
actSize = 50;
actID = 203;
actLen = 360;
...
for n = 1:actInstances
    st = actStart + ((n - 1) * actNext);
     fprintf('Activity occuring at %d in room %d with %d people.\n', st, ...
            actID, actSize);
    sim1c_InitProcesses( ...
        actID, ...            %node id
        st, ...               %tStart1
        st + actLen, ...      %tStart2
        actSize, ...          %nRoom
        st + actLen + actWait, ...    %tEnd1
        st + actLen*2 + actWait);       %tEnd2
end
% 
% actInstances = 1;
% actSize = 50;
...
% %All activities to the auditorium
% actInstances = 1;
% actSize = 50;
% actID = 200;
% actLen = 300;
...
% sID = size(noiseIDs);
% sID = sID(2);
% sValid = TMAX - (noiseLen * 3);
% 
% %Add noise as needed.
% for n = 1:noiseInstances
%     id = floor(rand() * sID) + 1;
%     st = floor(rand() * sValid) + 1;
%     nRoom = floor(rand()*noiseSize) + 1;
%     
%     sim1c_InitProcesses( ...
%         noiseIDs(id), ...     %node id
%         st, ...               %tStart1
...
%         st + noiseLen*3);     %tEnd2
%     
%     fprintf('Noise occuring at %d in room %d with %d people.\n', st, ...
%             noiseIDs(id), nRoom);
% end
