...
%
%
%Runs a Time Delayed Neural Network model for a given dataset and computes 
%statistics for the dataset.  Also creates a set of images.

clear all;

dataSet = 3;
model = 3; %Set model to TDNN
...
%data.trainData = 2*(data.trainData - min(data.trainData))/(max(data.trainData) - min(data.trainData)) - 1;
%data.testData = 2*(data.testData - min(data.testData))/(max(data.testData) - min(data.testData)) - 1;

%Train given model
aMod = bcf.models.TDNN(MyConstants.TDNN_PARAMETERS{dataSet}(1), MyConstants.TDNN_PARAMETERS{dataSet}(2));
aMod.train(data.trainData, 1);

% %First test data for white noise
% trainI = aMod.forecastAll(data.trainData, 1);
testF = aMod.forecastAll(data.testData(1, fStart:fEnd), 1);
...
res = testF - data.testData(fStart:fEnd);
...
title({'Example of TDNN forecasting capability'}, 'FontSize', 20, 'FontName', MyConstants.FONT_TYPE);
set(gca,'XTickLabel',[]);
legend('Scaled raw test data', 'One step TDNN forecast');
saveas(fig, ...
...
for h = 1:MyConstants.HORIZON
    ...
    masehist(3, h) = mase(data.testData(1, fStart:fEnd), ...
                             testF);
    fprintf(1, 'rmse: %f   sseonan: %f\n', rmsehist(3, h), sseonanhist(3, h));
end

horizons.tdnn{1} = rmsehist;
horizons.tdnn{2} = masehist;
horizons.tdnn{3} = ponanhist;
horizons.tdnn{4} = rmseonanhist;
horizons.tdnn{5} = sseonanhist;
horizons.tdnn{MyConstants.TRAINDATA_CELL_INDEX} = trainForecasts;
horizons.tdnn{MyConstants.TESTDATA_CELL_INDEX} = testForecasts;
