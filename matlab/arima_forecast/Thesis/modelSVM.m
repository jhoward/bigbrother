...
horizons.svm{1} = rmsehist;
horizons.svm{2} = masehist;
horizons.svm{3} = ponanhist;
horizons.svm{4} = rmseonanhist;
horizons.svm{5} = sseonanhist;
horizons.svm{MyConstants.TRAINDATA_CELL_INDEX} = trainForecasts;
horizons.svm{MyConstants.TESTDATA_CELL_INDEX} = testForecasts;

save(MyConstants.HORIZON_DATA_LOCATIONS{dataSet}, 'horizons');


%fig = plotPonan(testRes(1, startTime:endTime), stds, false);
%plot(trainRes(startTime:endTime))
