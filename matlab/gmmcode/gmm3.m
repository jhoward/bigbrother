...
numPossible = 2^k - 1;  % Include all but the instance where all activities 
                        % are off
                        
for n=1:N
    
    % Pick a number between 1 and 2^k (inclusive)
    
    u = floor(rand * numPossible + 1);
    value = dec2bin(u);
    
    totalMean = zeros(1, 2);
    totalVariance = zeros(2, 2);
    
    for j = 1:size(value, 2)
        if value(j) == '1'
            totalMean = totalMean + uK(j, :);
            totalVariance = totalVariance + CK(:, :, j);
        end
    end
    
    
    %Grab all ones from the binary decomposition of the number
